...
describe('BillingService', () => {
  ...
  let nfeService: jest.Mocked<NfeService>;
  ...
  beforeEach(async () => {
    ...
    nfeService = module.get(NfeService) as any;
  ...
  describe('processInvoicePayment', () => {
    it('should do nothing if invoice not found', async () => {
      prisma.invoice.findUnique = jest.fn().mockResolvedValue(null) as any;
    ...
    it('should process payment and update invoice when succeeded', async () => {
      prisma.invoice.findUnique = jest.fn().mockResolvedValue({
        ...
        amount: 100,
        reference: 'Ref',
        client: { billingEmail: 'test@test.com', financialContact: null } as any,
      } as any);
      ...
      expect(nfeService.emitInvoice).toHaveBeenCalledWith(
        'inv-1',
        100,
        expect.any(Object),
      );
    ...
    it('should update invoice with transaction info when not succeeded', async () => {
      prisma.invoice.findUnique = jest.fn().mockResolvedValue({
        ...
        amount: 100,
        reference: 'Ref',
        client: { billingEmail: null, financialContact: 'finance@test.com' } as any,
      } as any);
  ...
  describe('confirmPayment', () => {
    ...
    it('should update invoice and reactivate blocked client', async () => {
      prisma.invoice.findUnique = jest.fn().mockResolvedValue({
        ...
        status: 'PENDENTE',
        clientId: 'client-1',
        client: { id: 'client-1', status: 'SUSPENSO' } as any,
      } as any);
  ...
  describe('checkAndConfirmPayment', () => {
    it('should call confirmPayment when payment succeeded and invoice exists', async () => {
      ...
      const confirmSpy = jest.spyOn(service, 'confirmPayment').mockResolvedValue(undefined as any);
    ...
    it('should not throw when invoice not found', async () => {
      ...
      prisma.invoice.findFirst = jest.fn().mockResolvedValue(null) as any;
    ...
    it('should handle errors from paymentsService.checkStatus gracefully', async () => {
      paymentsService.checkStatus = jest
        .fn()
        .mockRejectedValue(new Error('gateway error')) as any;
